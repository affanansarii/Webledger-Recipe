...
const Profile = () => {
    ...
    const removeFavItems = async (foodId) => {
        try {
            const res = await axios.delete(`${BASEURL}/fav/${foodId}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
    ...
    return (
        <div
            className="border text-center max-w-3xl grid grid-cols-1 md:grid-cols-3 gap-8"
        ...
        >
            {/* <div className="bg-white shadow-xl rounded-lg p-6 text-center border border-gray-200">
                ...
                <p className="text-gray-500 mt-2">
                    {data?.user?.location || "Location Not Provided"}
                </p>
            ...
            <div className="md:col-span-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-6">
                    {data?.allFav?.items?.map((recipe) => (
                        <div
                            key={recipe.id}
                            className="relative group rounded-lg overflow-hidden shadow-lg bg-white transform hover:scale-105 transition duration-300"
                        >
                            <Link
                                to={`/recipe/${recipe?.foodId}`}
                                className="block"
                            >
                                <img
                                    src={
                                        recipe.image ||
                                        "https://tinyurl.com/458yz4wy"
                                    }
                                    alt={recipe.title}
                            ...
                            <button
                                className="absolute top-2 right-2 bg-red-500 text-white p-2 rounded flex items-center gap-1"
                                style={{ padding: "6px" }}
